...
// Komponen StatCard (dengan ukuran yang sudah diperkecil)
...
const NotificationCard = ({ title, icon, notifications, emptyText, viewLink, iconBgColor, iconColor }: { title: string; icon: React.ReactNode; notifications: any[]; emptyText: string; viewLink: string; iconBgColor: string; iconColor: string; }) => { const getNotificationDetails = (item: any) => { const student = item.student || item.submission?.student; return { id: item.id, fullName: student?.fullName || 'N/A', jurusan: student?.jurusan || 'N/A' }; }; return ( <div className="bg-white p-6 rounded-lg shadow-sm border h-full"> <div className={`flex items-center gap-3 mb-4 pb-3 border-b border-gray-200`}> <div className={`p-2 rounded-full ${iconBgColor}`}> <div className={iconColor}>{icon}</div> </div> <h3 className={`text-lg font-semibold ${iconColor}`}>{title}</h3> </div> {notifications.length > 0 ? ( <ul className="space-y-3 max-h-60 overflow-y-auto"> {notifications.map(item => { const { id, fullName, jurusan } = getNotificationDetails(item); return ( <li key={id} className="flex items-center justify-between p-3 bg-gray-50 rounded-md hover:bg-gray-100"> <div> <p className="font-medium text-sm text-gray-800">{fullName}</p> <p className="text-xs text-gray-500">{jurusan.replace('_', ' ')}</p> </div> <Link href={`${viewLink}/${jurusan}`} className="text-sm font-semibold text-blue-600 hover:underline flex items-center gap-1"> Lihat <FiArrowRight size={14}/> </Link> </li> ); })} </ul> ) : ( <div className="flex flex-col items-center justify-center h-48 text-center"> <FiBell className="w-10 h-10 text-gray-300 mb-2"/> <p className="font-semibold text-gray-500">{emptyText}</p> <p className="text-xs text-gray-400">Notifikasi baru akan muncul di sini.</p> </div> )} </div> ); };
...
export default function AdminDashboardClient({ titleSubmissions, proposalSubmissions, hasilSubmissions }: AdminDashboardProps) {
  ...
  const filterByPeriod = (data: any[]) => { /* ... Logika filter tidak berubah ... */ return data.filter(item => { const itemDate = new Date(item.createdAt); const itemYear = itemDate.getFullYear(); const itemMonth = itemDate.getMonth() + 1; const filterYear = parseInt(filters.tahun); switch (filters.periode) { case 'harian': return itemDate.toDateString() === new Date(filters.tanggal).toDateString(); case 'bulanan': return itemMonth === parseInt(filters.bulan) && itemYear === filterYear; case 'tahunan': return itemYear === filterYear; case 'ajaran': if (filters.semester === 'ganjil') return (itemYear === filterYear && itemMonth >= 8) || (itemYear === filterYear + 1 && itemMonth <= 1); return itemYear === filterYear + 1 && itemMonth >= 2 && itemMonth <= 7; default: return true; } }); };
  ...
  const verifiedTitles = useMemo(() => periodFilteredTitles.filter(s => s.status !== 'TERKIRIM'),[periodFilteredTitles]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* --- PERUBAHAN UTAMA: Pindahkan Notifikasi ke Bagian Atas --- */}
      <div>
        <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <FiBell /> Notifikasi Pengajuan Hari Ini
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <NotificationCard title="Pengajuan Judul Baru" icon={<FiFilePlus size={20}/>} notifications={todayTitleNotifications} emptyText="Tidak ada pengajuan judul hari ini." viewLink="/dashboard/admin/verifikasi" iconBgColor="bg-blue-100" iconColor="text-blue-600" />
          <NotificationCard title="Pendaftaran Sem. Proposal" icon={<FiClipboard size={20}/>} notifications={todayProposalNotifications} emptyText="Tidak ada pendaftaran proposal hari ini." viewLink="/dashboard/admin/sempro" iconBgColor="bg-green-100" iconColor="text-green-600" />
          <NotificationCard title="Pendaftaran Sem. Hasil" icon={<FiCheckSquare size={20}/>} notifications={todayHasilNotifications} emptyText="Tidak ada pendaftaran hasil hari ini." viewLink="/dashboard/admin/semhas" iconBgColor="bg-orange-100" iconColor="text-orange-600" />
        </div>
      </div>
