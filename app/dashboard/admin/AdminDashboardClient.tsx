...
export default function AdminDashboardClient({ titleSubmissions, proposalSubmissions, hasilSubmissions }: AdminDashboardProps) {
  ...
  const filterByPeriod = (data: any[]) => {
    return data.filter(item => {
      ...
      const filterYear = parseInt(filters.tahun);

      switch (filters.periode) {
        case 'harian':
          return itemDate.toDateString() === new Date(filters.tanggal).toDateString();
        case 'bulanan':
          return itemMonth === parseInt(filters.bulan) && itemYear === filterYear;
        case 'tahunan':
  ...
  const chartData = useMemo(() => {
    ...
    return {
      si: {
        ...
        proposal: verifiedProposals.filter(s => getJurusan(s) === 'SISTEM_INFORMASI').length,
        hasil: verifiedHasils.filter(s => getJurusan(s) === 'SISTEM_INFORMASI').length
      },
      mtk: {
        judul: verifiedTitles.filter(s => getJurusan(s) === 'MATEMATIKA').length,
        proposal: verifiedProposals.filter(s => getJurusan(s) === 'MATEMATIKA').length,
        hasil: verifiedHasils.filter(s => getJurusan(s) === 'MATEMATIKA').length
      }
    };
  }, [verifiedTitles, verifiedProposals, verifiedHasils]);
