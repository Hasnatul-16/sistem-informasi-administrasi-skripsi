...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (semester === 'GANJIL') {
      startDate = new Date(parseInt(tahun), 7, 1); // Agt - Feb
      endDate = new Date(parseInt(tahun) + 1, 1, 28, 23, 59, 59);
    } else {
      startDate = new Date(parseInt(tahun), 2, 1); // Mar - Jul
    ...
    const statsMap = new Map<string, { nama: string; nip: string; totalPembimbing1: number; totalPembimbing2: number; totalBimbingan: number }>();
    ...
    approvedJudul.forEach(j => {
      if (j.pembimbing1 && statsMap.has(j.pembimbing1)) {
        statsMap.get(j.pembimbing1)!.totalPembimbing1 += 1;
      }
      if (j.pembimbing2 && statsMap.has(j.pembimbing2)) {
        statsMap.get(j.pembimbing2)!.totalPembimbing2 += 1;
      }
    });
    ...
    const data = Array.from(statsMap.values())
      .map(stat => ({
        ...stat,
        totalBimbingan: stat.totalPembimbing1 + stat.totalPembimbing2,
      }))
      .filter(d => d.totalBimbingan > 0);
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          ...
          <p class="title">LAPORAN DAFTAR DOSEN PEMBIMBING</p>
            ...
            <tbody>
              ${data.map((dosen, index) => `
                <tr>
                  ...
                  <td>${dosen.nama}</td>
                  <td>${dosen.nip}</td>
                  <td>${dosen.totalPembimbing1} </td>
                  <td>${dosen.totalPembimbing2} </td>
                  <td>${dosen.totalBimbingan} </td>
                </tr>
              `).join('')}
            </tbody>
          </table>
    ...
    try {
      ...
      return new NextResponse(new Uint8Array(pdfBuffer), {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="Laporan_Pembimbing_${jurusan}_${periode.replace(' ', '_')}.pdf"`,
        },
      });
    } catch (error) {
